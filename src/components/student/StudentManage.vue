...
import { message, Modal, PaginationProps, SelectOption, TableColumnProps, TableProps } from "ant-design-vue"
...
const deleteSelection = () => {
  Modal.confirm({
    ...
    onOk: () => {
      ...
      selection.value.forEach((k: string) => {
        let s = k.split("-")
  ...
  <a-card>
    ...
    <a-table
      ...
      :pagination="{
        ...
        current: pageNum,
        pageSize: pageSize,
        'onUpdate:current': (current) => (pageNum = current),
        'onUpdate:pageSize': (size) => (pageSize = size),
        onChange: fetchData,
      }"
    />
  </a-card>
